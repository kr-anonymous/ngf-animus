...
print("Clean Test Accuracy of the Given Model (Before Initialization):", 100* cl_test_acc)

if args.backdoor_type != 'benign':
    po_test_loss, po_test_acc = test(model=net, criterion=criterion, data_loader=poison_test_loader)
    print("Attack Success Rate of the Given Model (Before Initialization):", 100*po_test_acc)


## Randomly Initialize the Last Layer parameters
torch.nn.init.xavier_uniform_(net.linear.weight)
params_finetune = [param for name, param in net.named_parameters() if "linear" in name]
...
if args.milestone is None:
    lr_scheduler = MultiStepLR(optimizer, milestones=[int(nb_repeat*0.3), int(nb_repeat*0.75)], gamma=0.1)
else:
...
def main():
  ...
  ## Validate the model 
  cl_test_loss, cl_test_acc = test(model=net, criterion=criterion, data_loader=clean_test_loader)
  print("Clean Test Accuracy of the Given Model (After Random Initialization):", 100*cl_test_acc)

  if args.backdoor_type != 'benign':
      po_test_loss, po_test_acc = test(model=net, criterion=criterion, data_loader=poison_test_loader)
      print("Attack Success Rate of the Given Model (After Random Initialization):", 100*po_test_acc)
  ...
  for i in range(nb_repeat):
    ...
    print("Clean Test Accuracy (After Purification):", 100*cl_test_acc)
    print("Attack Sucess Rate (After Purification):", 100*po_test_acc)
...
def Train(epoch, net):
    ...
    for batch_idx, (inputs, targets) in prog_bar:
        ...
        if optim_name in ['ngf'] and optimizer.steps % optimizer.TCov == 0:
            ## Compute True Fisher
